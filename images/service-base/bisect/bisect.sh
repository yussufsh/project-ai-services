...
run_step() {
    local label="$1" docling="$2" core="$3" ibm="$4" slim="$5" parse="$6" rapidocr="$7"

    echo ""
    echo "========================================"
    echo "STEP: $label"
    echo "  docling==$docling  core==$core  ibm-models==$ibm  parse==$parse  rapidocr==$rapidocr"
    echo "========================================"
...
#          parse 5.x  ibm-models 3.x  rapidocr 3.8.x
run_step "step1-baseline"  2.95.0  2.98.0 3.15.0 2.95.0  5.11.0  3.8.1

# Step 2: docling-parse jumps 5.x -> 6.x (Blend2D renderer)
#          ibm-models 3.x  rapidocr 3.8.x
run_step "step2-parse6x"   2.100.0 2.98.0 3.15.0 2.100.0 6.2.0   3.8.1

# Step 3: still parse 6.x
run_step "step3-parse6x-2" 2.105.0 2.98.0 3.15.0 2.105.0 6.2.0   3.8.1

# Step 4: docling-parse jumps 6.x -> 7.x (full C++ rewrite)
#          rapidocr bumped to 3.9.x (docling 2.110.0 requires >=3.9.1)
run_step "step4-parse7x"   2.110.0 2.98.0 3.15.0 2.110.0 7.21.0  3.9.2

# Step 5: still parse 7.x, same ibm-models
run_step "step5-parse7x-2" 2.115.0 2.98.0 3.15.0 2.115.0 7.21.0  3.9.2

# Step 6: ThreadedDoclingParseDocumentBackend becomes default (docling 2.123.0)
run_step "step6-threaded"  2.123.0 2.98.0 3.15.0 2.123.0 7.21.0  3.9.2

# Step 7: ibm-models jumps 3.x -> 4.x (TableFormer v2 O(1) post-proc fix)
run_step "step7-ibm4x"     2.125.0 2.98.0 4.0.3  2.125.0 7.21.0  3.9.2

# Step 8: final target (PR #1490)
run_step "step8-final"     2.127.0 2.98.0 4.0.3  2.127.0 7.21.0  3.9.2
